...
#endif
#endif /*VENDOR_EDIT*/
//huqiao@EXP.BasicDrv.Basic 2016/5/3 add for home key 
#ifdef VENDOR_EDIT_15311
static struct workqueue_struct *home_wq = NULL;
...
//huqiao@EXP.BasicDrv.Basic 2016/5/3 add for home key 
#ifdef VENDOR_EDIT
static struct workqueue_struct * kpd_volumekey_down_wq= NULL;
struct delayed_work  kpd_volumekey_down_work;
#endif
...
//huqiao@EXP.BasicDrv.Basic 2016/5/3 add for home key 
#ifdef VENDOR_EDIT
static void kpd_volumekey_down_wq_work_func(struct work_struct *work)
{
	int gpio_status;
	msleep(8000);
	gpio_status = mt_get_gpio_in(GPIO_VOLUMEKEY_DOWN);
	if(gpio_status ==0)
		{
...
//huqiao@EXP.BasicDrv.Basic 2016/5/3 add for home key 
	...
	if(vol_down_press == 0)
		{
		queue_delayed_work(kpd_volumekey_down_wq, &kpd_volumekey_down_work,  0);
		
	}
	else
		__cancel_delayed_work(&kpd_volumekey_down_work);
#endif
...
//huqiao@EXP.BasicDrv.Basic 2016/5/3 add for home key 
#ifdef VENDOR_EDIT_15311
static void home_wq_work_func(struct work_struct *work)
{
	int gpio_status;
	msleep(5000);
	gpio_status = mt_get_gpio_in(GPIO_HOME);
	if(gpio_status ==0)
		{
		mt_set_gpio_mode(GPIO_HOME, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_HOME, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_HOME, 1);
		home_irq = mt_gpio_to_irq(GPIO_HOME);
		request_irq(home_irq, (irq_handler_t)kpd_home_eint, IRQF_TRIGGER_LOW, KPD_HOME_NAME,  NULL);
		mt_eint_set_hw_debounce(GPIO_HOME, 32000);
		}		
}
#endif
static irqreturn_t kpd_home_eint(int irq, void *dev_id)
...
//huqiao@EXP.BasicDrv.Basic 2016/5/3 add for home key 
	...
	if(home_press == 0)
		{
		queue_delayed_work(home_wq, &home_work,  0);
...
//huqiao@EXP.BasicDrv.Basic 2016/5/3 add for home key 
#ifdef VENDOR_EDIT_15311
	home_wq = create_singlethread_workqueue("home_wq");
	INIT_DELAYED_WORK(&home_work, home_wq_work_func);
#endif
//huqiao@EXP.BasicDrv.Basic 2016/5/3 add for home key 
#ifdef VENDOR_EDIT
	kpd_volumekey_down_wq = create_singlethread_workqueue("kpd_volumekey_down_wq");
	INIT_DELAYED_WORK(&kpd_volumekey_down_work,kpd_volumekey_down_wq_work_func );
#endif	
