...
#ifndef VENDOR_EDIT
//xiaohua.tian@EXP.driver, modify the default rate for msensor, 2016-5-27
#define AKM09911_DEFAULT_DELAY	100
#else
#define AKM09911_DEFAULT_DELAY	20
#endif
...
extern struct mag_hw *akm09911_get_cust_mag_hw(void);
/*----------------------------------------------------------------------------*/
...
static ssize_t show_magnet_close(struct device_driver *ddri, char *buf)
{
    ...
    int result = -1;
    for (i = 0; i < 50; i++){
		ret = FctShipmntTestProcess_Body();
		if (ret == 1)
		{
		    result = 0;
		    break;
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#ifdef AKM_Pseudogyro
	struct hwmsen_object sobj_gyro, sobj_rote, sobj_grav, sobj_accel;
	#endif
	MAGN_FUN();
	...
	sobj_rote.self = data;
    sobj_rote.polling = 1;
    sobj_rote.sensor_operate = akm09911_rotation_vector_operate;
    if((err = hwmsen_attach(ID_ROTATION_VECTOR, &sobj_rote)))
    {
        printk(KERN_ERR "attach fail = %d\n", err);
        goto exit_kfree;
    }
	...
	sobj_grav.self = data;
    sobj_grav.polling = 1;
    sobj_grav.sensor_operate = akm09911_gravity_operate;
    if((err = hwmsen_attach(ID_GRAVITY, &sobj_grav)))
    {
        printk(KERN_ERR "attach fail = %d\n", err);
        goto exit_kfree;
    }
	
	sobj_accel.self = data;
    sobj_accel.polling = 1;
    sobj_accel.sensor_operate = akm09911_linear_accelration_operate;
    if((err = hwmsen_attach(ID_LINEAR_ACCELERATION, &sobj_accel)))
    {
        printk(KERN_ERR "attach fail = %d\n", err);
        goto exit_kfree;
    }
	...
	register_device_proc("Sensor_msensor", AKM09911_DEV_NAME, "MICROCHIP");
	#endif//VENDOR_EDIT
